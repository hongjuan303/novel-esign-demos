  ...
  return (
    <div className="product author-app">
      ...
      <main className="author-content">
        ...
        <div className="profile-ready"><b>✓ 签约资料已完善</b><span>实名、身份证、手机号、联系地址与收款账户已完成，可申请签约</span><button>查看资料</button></div>
        ...
        <section className="work-grid">
          <article className="work-card featured">
            ...
            <div className="work-info">
              ...
              <div className="contract-tip">
                ...
                {state === "待作者签署" && <><b>电子合同待作者签署</b><span>请在 2026-07-30 前完成签署</span></>}
                {state === "待平台签署" && <><b>作者已签署</b><span>合同正等待平台方签署，完成后自动归档</span></>}
              ...
              <div className="work-actions">
                ...
                {state === "待平台签署" && <button className="secondary" onClick={() => setSignModal(true)}>查看签署进度</button>}
                ...
                {state === "待发起" && <button className="secondary">查看签约资料</button>}
          ...
          <article className="work-card">
            ...
            <div className="work-info"><div className="work-heading"><h2>消失的女儿</h2><Badge>审核中</Badge></div><p>10,029字 ｜ 投稿时间：2026-07-22</p><div className="contract-tip muted">编辑正在审阅作品，请耐心等待审核结果</div><div className="work-actions"><button className="secondary">查看投稿详情</button><button className="text-button">小说详情</button></div></div>
          </article>
          <article className="work-card">
            <div className="book-cover sand"><span>雾港<br/>来信</span></div>
            <div className="work-info"><div className="work-heading"><h2>雾港来信</h2><Badge tone="gray">草稿</Badge></div><p>8,832字</p><div className="contract-tip muted"><b>作品签约信息未完整，暂不能申请签约</b><span>还需补充首次发表情况、完成时间和内容简介</span></div><div className="work-actions"><button className="primary">完善作品信息</button><button className="secondary">修改作品</button><button className="text-button danger">删除</button></div></div>
          </article>
        </section>
      </main>
      {signModal && (
        <div className="overlay" onClick={() => setSignModal(false)}>
          <div className="author-contract-modal" onClick={event => event.stopPropagation()}>
            ...
            <h2>{state === "待作者签署" ? "签署前确认" : "电子合同详情"}</h2>
            <p>《声声已离商音近》短篇小说版权合同（保底）</p>
            <div className="contract-summary">
              <div><span>签约方式</span><b>保底＋分成</b></div><div><span>税前保底费用</span><b className="money">¥12,000.00</b></div>
              <div><span>甲方</span><b>杭州宝茂（企业全称待确认）</b></div><div><span>乙方</span><b>石＊京（笔名：溪源）</b></div>
              <div><span>签署顺序</span><b>作者先签 → 平台后签</b></div><div><span>签署截止</span><b>2026-07-30 23:59</b></div>
            </div>
            <div className="signature-scope"><b>本次需完成 2 处签名</b><span>① 主合同签署页　② 附件《版权转让声明函》</span></div>
            <div className="safe-note">签署将在腾讯电子签安全页面完成。作者资料已在申请签约前完成；返回后最终状态以服务端回调为准。</div>
            {state === "待作者签署" && <>
              <label className="agreement"><input type="checkbox" defaultChecked /> 我已核对合同主体、保底费用和分成规则，并同意前往腾讯电子签签署</label>
              <button className="primary wide" disabled={signing} onClick={completeAuthorSign}>{signing ? "正在同步作者签署结果…" : "前往腾讯电子签"}</button>
            </>}
            {state === "待平台签署" && <button className="secondary wide" onClick={() => setToast("作者签名已完成，平台方尚未签署")}>作者已签，等待平台方</button>}
            {state === "签署完成" && <button className="primary wide" onClick={() => setToast("已下载签署完成的合同 PDF")}>下载已签合同</button>}
          </div>
        </div>
      )}
